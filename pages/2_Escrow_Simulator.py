...
with st.sidebar:
    ...
    st.caption("※スコアは将来AIが文案から自動採点（v2）。今は手動。")
    ir_on   = st.checkbox("IRインパクトを需給に反映する", value=False,
                          help="OFFなら従来のエスクロー試算のみ")
    ir_dir  = st.radio("材料の方向", ["ポジティブ", "ニュートラル", "ネガティブ"], index=0, horizontal=True)
    ir_str  = st.slider("強弱（インパクトの大きさ）", 0, 10, 6, 1,
                        help="0=ノイズ, 5=通常IR, 10=治験成功・大型提携など")
    ir_nov  = st.slider("新規性（サプライズ度）", 0, 10, 5, 1,
                        help="既に織り込み済みか、未知の好悪材料か")
    ir_mat  = st.slider("重要性（財務・戦略インパクト）", 0, 10, 5, 1)
    ir_cred = st.slider("信頼度（データ・第三者裏付け）", 0, 10, 6, 1,
                        help="低い＝プロモ的＝期待先行・反落しやすい")
    ir_guid = st.selectbox("見通し（ガイダンス）", ["なし", "上方", "中立", "下方"], index=0)
