...
| 企業登録（当社が会社名・担当者・メールを登録→仮パスワード発行。相手は初回ログインで3つの合意・パスワード変更・NIB・ハラール・ロゴを入力） | 作成済み |
...
6. 相手は3つの合意に同意するまで、依頼内容・添付ファイル・AI機能を一切使えません（データベース側でも制限しています）。
...
## 合意文について（必ずお読みください）
- 現在の合意文（版 2026-09-26）は `supabase/migrations/0010_hardening_terms_v2.sql` に登録し、`0011`・`0012`・`0017` で文言を修正しています（英語・インドネシア語が正本、日本語は参考訳）。一度でも同意された版は、データベース側で書き換えできないようにしています（改訂は新しい版として登録）。
