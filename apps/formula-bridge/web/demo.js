...
(() => {
  ...
  const supplierA = {
    ...
    tests: [{ lab: "DEMO LAB (not real)", item: "Patch test", method: "48h occlusive, n=30", result: "No irritation (demo)", date: "2026-09-18" }], files: [],
  };
  const seed = {
    ...
    assignments: [
      { id: "a-demo-a", project_id: P1, company_id: CA, status: "submitted", request_snapshot: snapshot, supplier: supplierA, requested_at: ago(168), submitted_at: ago(30), shipment: { carrier: "DHL", tracking: "DEMO1234567890", date: "2026-09-24", qty: "3 × 150 mL" }, shipped_at: ago(20), feedback: {}, feedback_at: null, updated_at: ago(20) },
    ...
    finals: [{ project_id: P1, adopted_assignment: "a-demo-a", base_formula: formula.map((r) => (r.inci === "Water" ? { ...r, pct: "83.65" } : { ...r })),
      ...
      plan: { productName: "（デモ）セラミド保湿化粧水", brand: "BIOT", target: "30〜40代・乾燥とハリ不足が気になる女性", price: "1,980円", cost: "240円", channel: "ドラッグストア・EC", launch: "2027年春", goal: "【要確認】", usp: "掛川の自社工場で製造するMade in Japan品質" },
      sup_ja: "（デモ）製品名：セラミド保湿化粧水\nコンセプト：セラミドNPとナイアシンアミドを配合した、毎日のバリアケア向けの軽い保湿ローション。\n特徴：ややとろみのあるジェルローションで素早くなじむ／無香料・パラベンフリー・エタノールフリー\n原料見積：1本あたり92円（デモ）\n第三者試験：パッチテスト（デモ・実在しないデータ）",
      finalized_at: null, updated_at: ago(3) }],
    plans: [{ project_id: P1, created_at: ago(1), plan: { title: "（デモ）セラミド保湿化粧水 新商品企画書", subtitle: "インドネシア原料メーカーとの共同開発・掛川工場で製造", date: "2026-09-25",
      slides: [["cover", "表紙", "案件：セラミド保湿化粧水／販売市場：日本・インドネシア", ["カテゴリ：化粧水 150mL", "作成日：2026-09-25"]],
        ...
        ["market_jp", "市場動向（日本）", "国内化粧品市場は拡大基調で、スキンケアが最大カテゴリ。", ["出典付きの調査データから作成（デモでは省略）"]],
  ...
  const client = {
    ...
    auth: {
      ...
      signUp: async () => ({ error: { message: "デモでは登録できません / Registration is disabled in the demo." } }),
